...
class JuiceboxFormatter implements JuiceboxFormatterInterface {
  ...
  public function getLibrary($force_local = FALSE, $reset = FALSE) {
    ...
    if (!$library || $reset) {
      ...
      if (!empty($query['jb-version']) && !$force_local) {
        ...
        $version_number = String::checkPlain($query['jb-version']);
  ...
  public function runCommonBuild(JuiceboxGalleryInterface $gallery, $settings, $data = NULL) {
    ...
    if ($global_settings['translate_interface']) {
      ...
      if (!empty($base_string)) {
        $gallery->addOption('languagelist', String::checkPlain($this->stringTranslation->translate($base_string)), FALSE);
      }
    }
   ...
   * {@inheritdoc}
   */
  public function buildEmbed(JuiceboxGalleryInterface $gallery, $settings, $xml_route = '', $xml_route_params = array(), $add_js = TRUE, $add_xml = FALSE, $contextual = array()) {
    // Merge all settings.
    ...
    if ($add_js) {
      ...
      // Pass-through the current query paramaters and a checksum on the XML
      // URL.
      $xml_query = array_merge(array($this->request->query->all(), 'checksum' => $gallery->getChecksum()), $embed_query_additions);
      $xml_url = $this->urlGenerator->generateFromRoute($xml_route, $xml_route_params, array('query' => $xml_query));
    ...
    $output['#suffix'] = String::format($output['#suffix']);
    return $output;
  }
  ...
  public function styleImageSrcData(FileInterface $image_file, $image_style, FileInterface $thumb_file, $thumb_style, $settings) {
    ...
    if (!$src_data['juicebox_compatible'] && $image_file->id() == $thumb_file->id()) {
      ...
      $src_data['thumb_url'] = $thumb_image_data['image_url'];
    }
    // Check if the linkURL should be customized based on settings.
    $src_data['link_url'] = $src_data['juicebox_unstyled_src'];
  ...
  protected function styleImage(FileInterface $file, $style, $check_compatible = TRUE) {
    ...
    $image_data['juicebox_unstyled_src'] = file_create_url($file->getFileUri());
    ...
    if ($check_compatible && !empty($library['compatible_mimetypes']) && !in_array($mimetype, $library['compatible_mimetypes'])) {
      ...
      if (file_exists($icon_path)) {
        $image_data['image_url'] = file_create_url($icon_path);
      }
      else {
        $image_data['image_url'] = file_create_url($icon_dir . '/general.png');
      }
    }
    // If the item is compatible, style it.
      ...
      if ($style == 'juicebox_multisize') {
        $sizes = array(
          'image_url_small' => $global_settings['juicebox_multisize_small'],
          'image_url' => $global_settings['juicebox_multisize_medium'],
          'image_url_large' => $global_settings['juicebox_multisize_large'],
        );
      }
      foreach ($sizes as $size => $style_each) {
        if (!empty($style_each)) {
          ...
          $image_data[$size] = $image_data['juicebox_unstyled_src'];
        }
      }
    }
    return $image_data;
  ...
  protected function setGalleryOptions(JuiceboxGalleryInterface $gallery, $settings) {
    ...
    foreach (array('jlib_galleryWidth', 'jlib_galleryHeight', 'jlib_backgroundColor', 'jlib_textColor', 'jlib_thumbFrameColor') as $name) {
      if (isset($settings[$name])) {
        ...
        $gallery->addOption(Unicode::strtolower($name_real), trim(String::checkPlain($settings[$name])));
      }
    }
    ...
    if (!empty($settings['manual_config'])) {
      ...
      foreach ($manual_options as $option) {
        ...
        if (!empty($option)) {
          ...
          $gallery->addOption(Unicode::strtolower($name), String::checkPlain($value));
        }
      }
    }
  }
