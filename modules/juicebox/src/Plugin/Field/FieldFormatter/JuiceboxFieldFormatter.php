...
class JuiceboxFieldFormatter extends ImageFormatterBase implements ContainerFactoryPluginInterface {
   ...
   */
  public function __construct(array $configuration, $plugin_id, array $plugin_definition, EntityManagerInterface $entity_manager, LinkGeneratorInterface $link_generator, JuiceboxFormatterInterface $juicebox) {
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    ...
    return new static($configuration, $plugin_id, $plugin_definition, $container->get('entity.manager'), $container->get('link_generator'), $container->get('juicebox.formatter'));
  }
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    // Add the common configuration optipns.
  ...
  public function viewElements(FieldItemListInterface $items) {
    ...
    $xml_id = 'field/' . $entity_type_id . '/' . $entity_id . '/' . $field_name . '/' . $display_name;
    $xml_args = explode('/', $xml_id);
    $xml_route = 'juicebox.xml_field';
    $xml_route_params = array('args' => $xml_id);
    // Try building the gallery and its XML.
    try {
      // Initialize the gallery.
      $gallery = $this->juicebox->newGallery($xml_args);
      // Build the gallery.
      $this->buildGallery($gallery, $items);
      // Build field-specific contextual links.
      $contextual = $this->buildContextualLinks($xml_id, $entity_type_id);
      // Create a render array with the gallery markup.
      $element[0] = $this->juicebox->buildEmbed($gallery, $this->getSettings(), $xml_route, $xml_route_params, $add_js, $this->isPseudoInstance(), $contextual);
    }
    catch (\Exception $e) {
  ...
  protected function buildGallery(JuiceboxGalleryInterface $gallery, FieldItemListInterface $items) {
    // Get settings.
    $settings = $this->getSettings();
    $global_settings = $this->juicebox->getGlobalSettings();
    ...
    foreach ($items as $delta => $item) {
      if ($item->isDisplayed() && !empty($item->target_id)) {
        ...
        if (!$src_data['juicebox_compatible'] && $settings['incompatible_file_action'] == 'show_icon_and_link') {
          ...
          $title = $this->linkGenerator->generate($anchor, Url::fromUri($src_data['link_url']));
        }
        else {
        ...
        $gallery->addImage($src_data, $title, $caption, $global_settings['apply_markup_filter']);
      }
    }
    // Run common build tasks. This is also where the general settings are
    // applied.
    $this->juicebox->runCommonBuild($gallery, $settings, $items, $this->isPseudoInstance());
  }
   ...
   *
   * @param string $xml_id
   *   The XML id that describes this field-based gallery.
   * @param string $entity_type_id
   *   The entity type for this field instance.
   * @return array
   *   An associated array of calculated contextual link information.
   */
  protected function buildContextualLinks($xml_id, $entity_type_id) {
    $contextual = array();
    ...
    if (!$this->isPseudoInstance()) {
      ...
      $contextual['juicebox_conf_field_' . $entity_type_id] = array(
        'route_parameters' => array(
          'view_mode_name' => ($display_entity->status() && !$display_entity->isNew()) ? $this->viewMode : 'default',
        ),
      );
