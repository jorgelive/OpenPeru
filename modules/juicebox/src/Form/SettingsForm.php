...
class SettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['juicebox_admin_intro'] = array(
      '#markup' => t("The options below apply to all Juicebox galleries. Note that most Juicebox configuration options are set within each gallery's unique configuration form and not applied on a global scope like the values here. Please be sure to clear your site's caches via the !performance settings after making any changes here.", array('!performance' => \Drupal::l(t('performance'), Url::fromRoute('system.performance_settings')))),
    );
    ...
    $form['multilingual']['base_languagelist_suggestion'] = array(
      ...
      '#description' => String::format('<pre>' . $library['base_languagelist'] . '</pre>'),
      '#states' => array(
        // Hide the settings when the translate option is disabled.
        'invisible' => array(
          ':input[name="translate_interface"]' => array('checked' => FALSE),
    ...
    $multisize_description = String::format($multisize_description);
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    // @todo: Be sure to clear any libraries API caches, or caches set in
    // JuiceboxFormatter:getLibrary() as some of the settings for this form may
    // influence the calculation of those values.
  }
