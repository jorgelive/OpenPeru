...
class JuiceboxSubRequestCase extends JuiceboxBaseCase {
  ...
  public function testSubRequestDependent() {
    ...
    $this->assertRaw('id="field--node--' . $node->id() . '--' . str_replace('_', '-', $this->instFieldName) . '---custom"', 'Embed code wrapper found.');
    ...
    $this->assertRaw('imageURL="' . String::checkPlain($test_image_url), 'Test image found in XML.' . $test_image_url);
    $this->assertRaw('thumbURL="' . String::checkPlain($test_thumb_url), 'Test thumbnail found in XML.' . $test_thumb_url);
